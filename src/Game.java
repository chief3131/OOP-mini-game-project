...
public class Game extends Canvas implements Runnable, KeyListener {
	...
	protected final static int GWIDTH = 1920/*(int) Toolkit.getDefaultToolkit().getScreenSize().getWidth()*/,
	...
	public Game() {
		...
		player = new Player(0, 0, LT.getPlayerRight(), ID.player, handler, LT);
	...
	public void update() {
		if(updateRun) {
			for(int i = 0; i < handler.objects.size(); i++) {
				if(handler.objects.get(handler.objects.size() - 1).ID == ID.player) {
		...
		for(int x = 0; x < w; x++) {
			for(int y = 0; y < h; y++) {
				...
				if(red == 255 && green == 0 && blue == 0) // castle block
					handler.addObject(new Block(x * SCALE, y * SCALE, LT.getCastle(), ID.block, BlockType.castle, LT, handler));
				else if(red == 125 && green == 50 && blue == 0) // normal dirt block
					handler.addObject(new Block(x * SCALE, y * SCALE, LT.getDirt(), ID.block, BlockType.dirt, LT, handler));
				else if(red == 0 && green == 200 && blue == 0) // dirt grass block
					handler.addObject(new Block(x * SCALE, y * SCALE, LT.getDirtGrass(), ID.block, BlockType.dirtGrass, LT, handler));
				else if(red == 200 && green == 200 && blue == 255) // transparent block
					handler.addObject(new Block(x * SCALE, y * SCALE, LT.getEmpty(), ID.block, BlockType.outsidebg, LT, handler));
				else if(red == 0 && green == 0 && blue == 255) { // player
					px = x;
					py = y;
				}
				else if(red == 255 && green == 200 && blue == 0) // lava top block
					handler.addObject(new Block(x * SCALE, y * SCALE, LT.getLavaTop(), ID.lava, BlockType.lavaTop, LT, handler));
				else if(red == 255 && green == 150 && blue == 0) // lava block
					handler.addObject(new Block(x * SCALE, y * SCALE, LT.getLava(), ID.lava, BlockType.lava, LT, handler));
				else if((red == 255 && green == 255 && blue == 0) || (red == 255 && green == 0 && blue == 255)) // win block
					handler.addObject(new Block(x * SCALE, y * SCALE, SCALE, SCALE, ID.win, BlockType.win, LT, handler));
			}
		}
	...
	public void keyPressed(KeyEvent evt) {
		...
		for(int i = 0; i < handler.objects.size(); i++) {
			...
			if(tempObject.ID == ID.player) {
				...
				else if(key == KeyEvent.VK_A) handler.left = true;
				else if(key == KeyEvent.VK_S) handler.down = true;
				else if(key == KeyEvent.VK_D) handler.right = true;
			}
		}
	...
	public void keyReleased(KeyEvent evt) {
		...
		for(int i = 0; i < handler.objects.size(); i++) {
			...
			if(tempObject.ID == ID.player) {
				...
				else if(key == KeyEvent.VK_A) handler.left = false;
				else if(key == KeyEvent.VK_S) handler.down = false;
				else if(key == KeyEvent.VK_D) handler.right = false;
			}
		}
	}
