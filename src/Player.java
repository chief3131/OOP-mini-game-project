...
public class Player extends GameObject {
	...
	protected int maxGroundSpeed = 10, jumpSpeed = 16, maxAirXSpeed = 10, maxFallSpeed = 40; 
	protected double jumpXResistance = 0.9; 

	public Player(int x, int y, BufferedImage img, ID ID, Handler handler, LoadedTextures LT) {
		super(x, y, img, ID, LT);
		start(handler);
	}
	public Player(int x, int y, BufferedImage img, double boundpos, ID ID, Handler handler, LoadedTextures LT) {
		super(x, y, img, boundpos, ID, LT);
		start(handler);
	}
	public Player(int x, int y, int width, int height, ID ID, Handler handler, LoadedTextures LT) {
	...
	// M O V E M E N T
		...
		if (handler.right && !jumping) { 
			...
			//if(velY > 0) velX *= jumpXResistance*2;
			//velX = maxGroundSpeed;
